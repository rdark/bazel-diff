...
class CalculateImpactedTargetsInteractorModuleQueryTest : KoinTest {
  ...
  fun matchesCanonicalPlusFormRepo() { runBlocking {
    ...
    assertThat(queryCaptor.firstValue).contains("@@aspect_bazel_lib+//...")
    assertThat(queryCaptor.firstValue).doesNotContain("@@other+//...")
  ...
  fun matchesCanonicalTildeFormRepo() { runBlocking {
    ...
    verify(queryService).query(queryCaptor.capture(), eq(false))
    assertThat(queryCaptor.allValues).hasSize(1)
    assertThat(queryCaptor.firstValue).contains("@@abseil-cpp~20240722.0//...")
  } }
  ...
  fun matchesExtensionCreatedRepoViaMapping() { runBlocking {
    ...
    verify(queryService).query(queryCaptor.capture(), eq(false))
    assertThat(queryCaptor.allValues).hasSize(1)
    assertThat(queryCaptor.firstValue).contains("@@rules_jvm_external+//...")
    assertThat(queryCaptor.firstValue).contains("@@rules_jvm_external++maven+maven//...")
    assertThat(queryCaptor.firstValue).contains(" + ")
  } }
  ...
  fun doesNotMatchUnrelatedRepoBySubstring() { runBlocking {
    ...
    whenever(queryService.discoverRepoMapping()).thenReturn(emptyMap())
    whenever(queryService.query(any(), any())).thenAnswer { _ ->
      // Single unioned query contains only `@@cpp+//...`, returning `//foo:bar`.
    ...
    assertThat(queryCaptor.allValues).hasSize(1)
    assertThat(queryCaptor.firstValue).contains("@@cpp+//...")
    assertThat(queryCaptor.firstValue).doesNotContain("@@abseil-cpp+//...")
  ...
  fun transitiveModuleNotInRootMappingUsesNameFallback() { runBlocking {
    ...
    verify(queryService).query(queryCaptor.capture(), eq(false))
    assertThat(queryCaptor.allValues).hasSize(1)
    assertThat(queryCaptor.firstValue).contains("@@deep_transitive_dep~3.2.1//...")
  } }
